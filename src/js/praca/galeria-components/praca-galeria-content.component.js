class PracaGaleriaContentController {
  constructor($attrs, Praca, $scope, $stateParams, $document, $mdMedia, $mdMenu, $mdDialog, Toast, $log, $filter, $state, User) {
    "ngInject"
    ...
    Praca.get($stateParams.pk).then((data) => {
      $scope.praca = data
      Praca.getImages($stateParams.pk).then(arrResult => $scope.imagens = arrResult.reverse())
    })
    this.praca = $scope.praca;
    ...
    this.delete = (ev, pkImg) => {
      ...
      $mdDialog.show(confirm).then(() => {
        Praca.deleteImg($stateParams.pk, pkImg)
          .then(
            (response) => {
              $scope.paginatorData = $scope.paginatorData.filter((obj) => obj.id != pkImg); // Retira do array de objetos a imagem deletada.
    ...
    $("body").on("click touchend", ".material-placeholder img", () => {
      const elmActive = $(".materialboxed.active");
      if (elmActive.length > 0) {
        ...
        <md-grid-tile ng-repeat="objPhoto in paginatorData"
          ...
          <md-grid-tile-footer>
              <h3>
                ...
                right: 0;
                top: 5px;"
                show-as-manager="true" pracaid="{{$ctrl.praca.id_pub}}" pracagestor="{{$ctrl.praca.gestor.user_id_pub}}">
